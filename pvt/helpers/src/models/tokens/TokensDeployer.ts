...
import { deploy, getArtifact } from '../../contract';
...
class TokensDeployer {
  ...
  async deployToken(params: RawTokenDeployment): Promise<Token> {
    ...
    let instance;
    if (symbol !== 'WETH') {
      const artifact = await getArtifact('TestToken');
      const factory = new ethers.ContractFactory(artifact.abi, artifact.bytecode, sender);
      instance = await factory.deploy(name, symbol, decimals);
